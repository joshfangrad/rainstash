...
export function checkBoolCookie(key, defaultState) {
...
export function getOrCreateEnabledItems() {
    let cookie = getCookie('enabledItems');
    if (cookie) {
        return JSON.parse(cookie);
    } else {
        //set default if there's no cookie for this
        setCookie('enabledItems', '["vanilla_items"]', 365);
